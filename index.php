    ...
    if(isset($_POST['submit'])){
      if(!empty($_POST['lastName']) && !empty($_POST['firstName']) && !empty($_POST['civility'])){
        ...
        <form action="#" method="POST"><!--Création formulaire qui ne s'affiche qu'au début et lorsque toutes les informations ne sont pas remplies-->
          ...
          <input type="text" name="lastName" id="lastName" placeholder="nom" /><!--Champs nom de famille-->
          <label for="firstName">Votre prénom: </label>
          <input type="text" name="firstName" id="firstName" placeholder="prénom" /><!--Champs prénom-->
          ...
          <input type="submit" name="submit" value="Soumettre" /><span class="error"><?= 'Tous les champs ne sont pas remplis !';?></span>
        </form>
        <?php
      }
  } else {
